...
void draw() {
  float amplitude = map(mouseY, 0, height, 0.5, -0.1);
    ...
    if (key=='r'){
      if (rec == 0 & play == 0){
        ...
        while (recf[pos] > 0){
          ...
          fill(0);
          if(reca[pos] > 0.001){
            ellipse(map(recf[pos], min, max, 0, width),map(reca[pos], 0.5, -0.1, 0, 600),3,3);
          }
          pos=pos+1;
        }
  ...
  if(rec == 1){
    ...
    fill(255);
    if(reca[pos] > 0.001){
      ellipse(map(recf[pos], min, max, 0, width),map(reca[pos], 0.5, -0.1, 0, 600),1,1);
    }
